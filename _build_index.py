...
<html lang="en">
  <head>
    ...
    <style>
      ...
      /* Section pills reuse the gallery's blue-pill header language.
         Results = solid blue (primary); browse-all = ghost outline (secondary). */
      .sec-pill {{ font-family: 'IBM Plex Mono', monospace; font-size: 16px; line-height: 1; padding: 11px 22px; border-radius: 999px; background: #3B82F6; color: #fff; white-space: nowrap; max-width: -moz-fit-content; max-width: fit-content; }}
      .sec-pill b {{ font-weight: 700; }}
      .sec-pill.ghost {{ background: #fff; color: #6b7280; border: 1.5px solid #CBD5E1; }}
      #results-bar {{ display: none; align-items: center; gap: 14px; margin-bottom: 22px; }}
      #results-bar.show {{ display: flex; }}
      #results-clear {{ margin-left: auto; flex: 0 0 auto; font-family: 'IBM Plex Mono', monospace; font-size: 13px; color: #6b7280; background: #fff; border: 1px solid #D2DAE1; border-radius: 999px; padding: 8px 16px; cursor: pointer; transition: color .12s, border-color .12s; }}
      #results-clear:hover {{ color: #374151; border-color: #9aa6b1; }}
      ...
      /* Browse-all — every non-matching asset; clearly set off as a secondary section. */
      #more-section {{ display: none; }}
      #more-section.show {{ display: block; }}
      #more-head {{ display: flex; align-items: baseline; flex-wrap: wrap; gap: 6px 14px; margin: 48px 0 22px; padding-top: 30px; border-top: 2px dashed #C7D2E0; }}
      #more-sub {{ font-family: 'Source Sans 3', sans-serif; font-size: 13px; color: #9aa6b1; }}
...
{chips_html}
    ...
    <div class="container px-5 py-10 mx-auto">
      <div id="results-bar">
        <span class="sec-pill" id="results-count"></span>
      ...
      <div id="more-section">
        <div id="more-head">
          <span class="sec-pill ghost" id="more-label">Browse all other Huggies</span>
          <span id="more-sub">everything that didn&rsquo;t match</span>
        </div>
